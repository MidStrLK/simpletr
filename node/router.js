...
function route(handle, pathname, response, postData, COLLECTION, TELEGRAM) {
    ...
    if (pathname === '/') {
        handle(response, handle, pathname, postData, COLLECTION, TELEGRAM);


    }else if (pathname.indexOf('/api/') === 0) {
        handle(response, handle, pathname.substr(4), postData, COLLECTION, TELEGRAM);
